...
/// that is not otherwise dominated by a gamete producible by any other genotype in the current
/// population given a number `n_loci` of loci, initial population `pop_0`, vector `rec_rate` of
/// recombination rates, and threshold probability `gamma`.
#[pyfunction]
...
/// augmenting the current population by the minimum cost crossing that can produce some gamete
/// that is not otherwise dominated by a gamete producible by any other genotype in the current
/// population given a distribute array `xs`, a vector `rec_rate` of recombination rates, and a
