...
fn eugene_rs(m: &Bound<'_, PyModule>) -> PyResult<()> {
    ...
    mg_naive1.add_function(wrap_pyfunction!(
        solvers::base_min_generations_enumerator::breeding_program_distribute_python,
        &mg_naive1
    )?)?;
    ...
    mg_naive2.add_function(wrap_pyfunction!(
        solvers::base_min_generations_enumerator_dominance::breeding_program_distribute_python,
        &mg_naive2
    )?)?;
    ...
    mg_segment.add_function(wrap_pyfunction!(
        solvers::base_min_generations_segment::breeding_program_distribute_python,
        &mg_segment
    )?)?;
